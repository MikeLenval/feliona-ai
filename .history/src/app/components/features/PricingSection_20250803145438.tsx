...
import type { PlanType, PricingPlan, PlanFeature } from '@/types';
...
const PricingCard = React.memo(({ 
  ...
  const handleSelect = useCallback(() => {
    trackPricingEvent('plan_select', 'pricing_section', plan.id);
    onSelect(plan.id);
  }, [plan.id, onSelect]);
  ...
  const isAdultPlan = ['intimate', 'enterprise'].includes(plan.id);
  const cardVariant = plan.popular ? 'pricing' : isAdultPlan ? 'adult' : 'glass';

  return (
    <Card
      variant={cardVariant}
      className={`pricing-card ${isHovered ? 'hovered' : ''} ${plan.popular ? 'popular' : ''} ${isAdultPlan ? 'adult-pricing-plan' : ''}`}
