...
const envSchema = z.object({
  ...
  // 🔧 PRODUCTION-SAFE CORS с validation и safe defaults
  CORS_ALLOWED_ORIGINS: z.string().default('https://feliona.ai,https://feliona.app'),
  // 🔧 BOUNDED CACHE MULTIPLIER для предотвращения excessive TTL
  CACHE_TTL_MULTIPLIER: z.coerce.number().min(1).max(3).default(1.5), // Bounded: max 3x instead of 5x
  // 🔧 EXPLICIT CACHE LIMITS для контроля memory usage
  RATE_LIMIT_CACHE_SIZE: z.coerce.number().min(1000).max(10000).default(3000), // Bounded increase
  RATE_LIMIT_MAX_TTL: z.coerce.number().default(1800000), // 30 min max TTL instead of unlimited
});
...
// 🔧 CRITICAL SAFETY: Startup CORS validation с comprehensive error handling
...
function isPublicRoute(pathname: string): boolean {
  ...
  // Проверяем статические файлы и системные пути
  ...
  // 🌍 Проверяем локализованные публичные маршруты
  for (const locale of SUPPORTED_LOCALES) {
    if (pathname === `/${locale}`) return true;
    
    // Локализованные публичные страницы
...
/**
 * 🔧 BOUNDED TTL: Cache TTL calculation с maximum limits для предотвращения excessive caching
 */
function calculateCacheTTL(windowMs: number): number {
  const calculatedTTL = Math.max(windowMs * env.CACHE_TTL_MULTIPLIER, 60000); // Минимум 1 минута
  // 🔧 CRITICAL BOUND: Maximum TTL limit для предотвращения stale data
  return Math.min(calculatedTTL, env.RATE_LIMIT_MAX_TTL);
}
